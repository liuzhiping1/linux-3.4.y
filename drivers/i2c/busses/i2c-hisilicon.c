...
void hi_i2c_set_rate(struct hi_i2c *pinfo)
{
	...
	writel(scl_h, pinfo->regbase + I2C_SCL_L_REG);
...
{
	unsigned int  work_satatus;
	unsigned int  auto_satatus;
	unsigned int  int_raw_satatus = 0;
	unsigned int  i = 0;
	int ret = 0;

	unsigned int dmac_finish;

	work_satatus = readl(pinfo->regbase + I2C_STATUS_REG);
	auto_satatus = readl(pinfo->regbase + I2C_AUTO_REG);

	while (!(IS_FIFO_EMPTY(auto_satatus) && IS_I2C_IDLE(work_satatus))) {
		if (i > I2C_WAIT_IDLE_TIME_OUT) {
			hi_err("wait i2c idle timeout!"\
					"auto_satatus: 0x%x, work_satatus: 0x%x\n",
					auto_satatus, work_satatus);
			ret = -1;
			break;
		}
		...
		hi_msg("===== i: %d, auto_satatus: 0x%x,"\
			"work_satatus: 0x%x.\n", i, auto_satatus, work_satatus);
	...
	int_raw_satatus = readl(pinfo->regbase + I2C_INTR_RAW_REG);

	if ((int_raw_satatus & I2C_RAW_TX_ABORT) == I2C_RAW_TX_ABORT) {
		hi_err("transmit error, int_raw_satatus: 0x%x!\n",
				int_raw_satatus);
...
int hi_i2c_wait_txfifo_notfull(struct hi_i2c *pinfo)
{
	...
	while ((auto_satatus & I2c_AUTO_TX_FIFO_NOT_FULL)
			!= I2c_AUTO_TX_FIFO_NOT_FULL) {
		if (i > I2C_WAIT_TIME_OUT) {
			hi_err("wait timeout, auto_satatus: 0x%x!\n",
					auto_satatus);
			ret = -1;
			break;
		}
...
int hi_i2c_wait_rxfifo_notempty(struct hi_i2c *pinfo)
{
	...
	auto_satatus = readl(pinfo->regbase + I2C_AUTO_REG);
	while ((auto_satatus & I2C_AUTO_RX_FIFO_NOT_EMPTY)
		...
		unsigned int work_mode)
{
	...
	writel(0x0, pinfo->regbase + I2C_INTR_MASK_REG);
	/* clear err status */
